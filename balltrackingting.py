...
import time
import pandas as pd
from detect_obcopy import pixel_per_cm
...
pixdist = dist.euclidean((ax, ay1), (bx, by2)) ##400pix##distance of markers

vrdist = pixdist/pixel_per_cm ## distance of object wihtin the frame  in ppm
...
while True:
        ...
        if len(cnts) > 0:
            ...
            if (radius > 0 ) : 
                ...
                while int(ax) == int(x) in range(10-x,x+10): ##while centroid is passing through the line sart time
                        start_time = time.time()
                        print("Timer STARTED")
                        break
                        
                while int(ax) <= int(x): #while ball is inbetween lines
                    if int(bx) <= int(x)&int(bx+10) >= int(x): #if ball has just pased 2nd line or a little after make 2nd line green
